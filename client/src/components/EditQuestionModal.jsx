...
import { updateQuestion, getTags } from '../api';

export default function EditQuestionModal({ question, onClose, onQuestionUpdated }) {
  const predefinedTypes = ['Semester', 'Midterm', 'Class Test', 'Quiz', 'Assignment', 'Book Question'];
  const isCustomType = question.type && !predefinedTypes.includes(question.type);
  
  const [title, setTitle] = useState(question.title || '');
  const [notes, setNotes] = useState(question.notes || '');
  const [references, setReferences] = useState(question.references_text || '');
  ...
  const [type, setType] = useState(isCustomType ? 'Other' : (question.type || 'Semester'));
  const [customType, setCustomType] = useState(isCustomType ? question.type : '');
  const [tags, setTags] = useState(question.tags ? question.tags.map(t => t.name).join(', ') : '');
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in fade-in duration-300">
      <div className="bg-bg-surface rounded-[2.5rem] shadow-2xl w-full max-w-3xl max-h-[95vh] overflow-y-auto relative flex flex-col border border-border-main">
        ...
        <div className="p-8 border-b border-border-main flex justify-between items-start bg-bg-surface/80 backdrop-blur-md sticky top-0 z-10">
            <div>
                <h2 className="text-2xl font-black text-text-main tracking-tighter uppercase">Edit Metadata</h2>
                <p className="text-[10px] font-black text-text-muted uppercase tracking-widest mt-1">Refining the archives</p>
            </div>
            <button 
                onClick={onClose}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <div className="space-y-2">
                <select 
                  className="w-full px-4 py-3 bg-bg-main border border-border-main rounded-xl focus:ring-2 focus:ring-primary outline-none transition-all text-sm font-bold text-text-main appearance-none"
                  value={type}
